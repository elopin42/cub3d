...
void	set_glb_square(t_global *glb, int size_y, int size_x)
{
	if (size_x < size_y)
...
void	set_map_dimensions(t_global *glb)
{
	...
	if (glb->m_w * glb->m_h > 10000)
		return (glb->square_size = 0,
			printf("Minimap disabled: map too big (%dx%d)\n", glb->m_w,
				glb->m_h), (void)0);
	size_x = MINIMAP_MAX_WIDTH / glb->m_w;
	size_y = MINIMAP_MAX_HEIGHT / glb->m_h;
	set_glb_square(glb, size_y, size_x);
}
